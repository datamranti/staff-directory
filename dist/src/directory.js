...
function searchText(user) {
  return normalize([
    ...
    user.phoneNumbers, user.aliases, user.additionalEmails
  ].join(' '));
}
...
function cardTemplate(user) {
  ...
  return `
    <article class="staff-card" tabindex="0" data-staff-id="${escapeHtml(user.id || user.primaryEmail)}">
      <div class="staff-card-main">
        <div class="avatar">${escapeHtml(initials(user.fullName))}</div>
        <div class="staff-copy">
          <div class="staff-name-row"><h2>${escapeHtml(user.fullName || 'Unnamed staff')}</h2>${department}</div>
...
function openUser(user) {
  ...
  elements.drawerContent.innerHTML = `
    <div class="drawer-profile">
      ...
      ${user.department ? `<span class="department-chip">${escapeHtml(user.department)}</span>` : ''}
    </div>
...
export function initializeDirectory(options = {}) {
  callbacks = options;
  elements = {
    searchInput: $('searchInput'), departmentFilter: $('departmentFilter'), locationFilter: $('locationFilter'),
  ...
  [elements.departmentFilter, elements.locationFilter, elements.sortSelect].forEach(el => el.addEventListener('change', render));
